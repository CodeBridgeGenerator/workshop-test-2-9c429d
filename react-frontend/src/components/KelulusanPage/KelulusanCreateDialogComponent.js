...
const KelulusanCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            latihan: _entity?.latihan,pelulusan: _entity?.pelulusan,status: _entity?.status,komen: _entity?.komen,
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                <label htmlFor="pelulusan">Pelulusan:</label>
                <InputText id="pelulusan" className="w-full mb-3 p-inputtext-sm" value={_entity?.pelulusan} onChange={(e) => setValByKey("pelulusan", e.target.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["pelulusan"]) ? (
              <p className="m-0" key="error-pelulusan">
                {error["pelulusan"]}
              </p>
            ) : null}
          </small>
